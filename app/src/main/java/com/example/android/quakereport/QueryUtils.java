...
//    Add earthquake to list of earthquakes
    public static ArrayList<Earthquake> extractEarthquakes(String jsonResponse) {
        ...
        try {
            ...
            JSONObject jsonRootObject = new JSONObject(jsonResponse);//SAMPLE_JSON_RESPONSE

            //Get the instance of JSONArray that contains JSONObjects
            JSONArray jsonArrayFeatures = jsonRootObject.optJSONArray("features");

            //Iterate the jsonArray and print the info of JSONObjects
            for(int i=0;i<jsonArrayFeatures.length();i++){
                JSONObject jsonObjectIndex = jsonArrayFeatures.getJSONObject(i);

                JSONObject jsonObjectProperties = jsonObjectIndex.getJSONObject("properties");

                double mag = jsonObjectProperties.getDouble("mag");
    ...
    /**
     * Query the USGS dataset and return an {@link Earthquake} object to represent an array of earthquake
     */
